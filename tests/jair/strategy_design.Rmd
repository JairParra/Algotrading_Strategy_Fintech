...
N_window <- 12 
N_runs <- floor((N_months - N_window)/1)
N_sector_stocks <- 3 

# setup variables to record results 
portfolio <- rep(NA, length(sectors)*N_sector_stocks)
weights <- rep(1/length(portfolio), length(portfolio)) # initialize to 1/n
...
for(tau in seq(N_runs)){
  ...
  for(G in sectors){
    ...
    portfolio[i_replace] <- top_sector_stocks
    i_sector <- i_sector + 3
  }
