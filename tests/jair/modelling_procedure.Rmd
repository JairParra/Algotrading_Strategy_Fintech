...
Recall that the $\text{SECTOR_PROCEDURE}(G,\tau)$ function takes the argument $G$, which is the **sector name**, and **tau**, which is the current run in the backtesting. 
...
3. The `f_extract_train_val_features()` function extracts the appropriate window of data corresponding to the $\tau$, with the appropriate window size, for all sectors. `val_lag` specifics the number of months to be assigned to the validation set. 
...
best_feat_list
```
The result of this object is a list `best_feat_list` in this case, containing two objects: 
- `featnames`: a list of features selected. 
- `fmla`: An R formula (for regression, etc) 

**NOTE:** This is just for illustration and to visualize the data. The actual feature selection is performed in a loop for every stock as illustrated in the next section. 
...
\right]
$$
First, we wil do the following: 
1. Specify the general formula 
2. Create the grid of parameters to use in the Elasticnet models 
...
for(ticker in sector_tickers){
  ...
  full_train <- list_train_val_sector[[ticker]]

  # Compute dynamic features 
  full_train <- f_extract_dynamic_features(full_train, 
  ...
  # sector_tracker[[ticker]]$data = rbind.xts(ticker_data_train, ticker_data_val) # uncomment for final!
