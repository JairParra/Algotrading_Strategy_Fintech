...
```
The result is the `list_train_val_sector` object, which is a list of lists. 
...
**NOTE:** 
- This is just for illustration and to visualize the data. The actual feature selection is performed in a loop for every stock as illustrated in the next section. 
- There will always be linear dependencies because of the ARIMA features. This is normal. 
...
f_select_top_stocks <- function(sector_tracker, n=3){
  ...
  # Concat in one list 
  top_tickers  <-c(top_sharpe, top_fore_rets) 
...
```{r}
# Cbtain data for the top n*2 stocks (best forecasted rets and best sharpe together)
f_select_top_stocks(sector_tracker = sector_tracker, n = 2)
```
